...
sequelize.authenticate().then(
  ...
  err =>
    console.error(
      "We failed to establish a connection to the database, here's why: ",
      err
    )
);
...
Logs.sync().then(
  () => console.log('Table Logs synced successfully!'),
  err =>
    console.error(
      "Failed to sync Logs table to the database, here's why: ",
      err
    )
);
Errors.init(
...
Errors.sync().then(
  () => console.log('Table Errors synced successfully!'),
  err =>
    console.error(
      "Failed to sync Errors table to the database, here's why: ",
      err
    )
);
Clusters.init(
...
Clusters.sync().then(
  () => console.log('Table Clusters synced successfully!'),
  err =>
    console.error(
      "Failed to sync GuildCount table to the database, here's why: ",
      err
    )
);
...
app.use((req, res, next) => {
  ...
  if (
    Object.keys(users).indexOf(auth.split(':')[0]) !== -1
      ? Object.values(users)[Object.keys(users).indexOf(auth.split(':')[0])] ===
        auth.split(':')[1]
      : false
  ) {
...
wss.on('connection', ws => {
  ...
  ws.on('message', d => {
    try {
      ...
      if (ws.ID !== undefined) {
        ...
        if (clusters.size === ws.clusterCount) {
          console.info(
            'All clusters are no longer connected! Notifying all remaining clusters!'
          );
    ...
    if (json.op === undefined || json === undefined) {
      console.info(
        'The connection did not send valid payload data, disconnecting!'
      );
      if (ws.ID !== undefined) {
        clusters.delete(ws.ID);
        if (clusters.size === ws.clusterCount) {
    ...
    switch (json.op) {
      case 2:
        // Identify payload
        if (
          (!json.d.token || json.d.clusterID === undefined,
          !json.d.clusterCount)
        ) {
          console.info(
            'The connection sent an invalid payload, disconnecting!'
          );
        ...
        if (json.d.clusterID >= json.d.clusterCount) {
          console.info(
            'The connection sent an invalid clusterID, disconnecting!'
          );
        ...
        if (clusters.has(json.d.clusterID)) {
          console.info(
            'A connection tried to identify with a clusterID that was already connected, disconnecting!'
          );
        ...
        if (
          Object.keys(users).indexOf(auth.split(':')[0]) !== -1
            ? Object.values(users)[
                Object.keys(users).indexOf(auth.split(':')[0])
              ] === auth.split(':')[1]
            : false
        ) {
          console.info(
            `Connection with username: ${
              auth.split(':')[0]
            } identified successfully as cluster: ${json.d.clusterID}`
          );
          ...
          clearTimeout(identifyTimeout);
          if (
            !clusters.every(wwss => wwss.clusterCount === json.d.clusterCount)
          ) {
            console.log(
              'New cluster count identified, disconnecting all clusters'
            );
      ...
      case 3:
        // Update all
        if (
          json.d.guildCount === undefined ||
          json.d.ping === undefined ||
          json.d.cpuUsage === undefined ||
          json.d.memUsage === undefined
        ) {
          console.info(
            'The connection sent an invalid payload, disconnecting!'
          );
        ...
        if (
          clusterDP.guildCount.filter(e => e !== undefined).length ===
          ws.clusterCount
        ) {
          Clusters.create({
      ...
      case 5:
        ...
        Logs.create({ id: new Date().toISOString(), data: json.d }).then(() =>
          ws.send(JSON.stringify({ op: 4, d: true }))
        );
        break;
      case 6:
        ...
        Errors.create({ id: new Date().toISOString(), data: json.d }).then(() =>
          ws.send(JSON.stringify({ op: 4, d: true }))
        );
        break;
      case 9:
        if (
          json.d.id === undefined ||
          json.d.uid === undefined ||
          json.d.id >= ws.clusterCount
        ) {
          console.info(
            'The connection sent an invalid payload, disconnecting!'
          );
        ...
        let uid = Date.now();
        clustersCom[ws.ID] = {
          request: { id: ws.ID, uid: json.d.uid },
          response: { id: json.d.id, uid }
        };
        clusters
          .get(json.d.id)
          .send(
            JSON.stringify({ op: 9, d: { id: ws.ID, data: json.d.data, uid } })
          );
        function getData(d2) {
          let json2;
          try {
            ...
            console.info(
              'The connection did not send JSON data, disconnecting!'
            );
          ...
          if (
            json2.oop === 10 &&
            json2.d.uid === clustersCom[ws.ID].response.uid &&
            json2.d.id === clustersCom[ws.ID].request.id
          ) {
            clusters.get(clustersCom[ws.ID].request.id).send(
              JSON.stringify({
                op: 10,
                d: {
                  uid: clustersCom[ws.ID].request.uid,
                  data: json2.d.data,
                  id: clustersCom[ws.ID].response.id
                }
              })
            );
...
setInterval(() => {
  clusters.forEach(ws => {
    if (!ws.isAlive) {
      ws.terminate();
      console.info(
        `Cluster ${ws.ID} did not respond to a ping, disconnecting.`
      );
      if (ws.ID !== undefined) {
        if (clusterDP[ws.ID]) delete clusterDP[ws.ID];
        if (clustersCom[ws.ID]) delete clustersCom[ws.ID];
